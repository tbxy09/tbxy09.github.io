...
#     name: str = Field(..., description="relative path of the file")
...
async def query_file_structure(random_string: str):
    root = Path("static") / random_string
    if root.is_dir():
...
async def download_files(projectID: str, filelist: str):
    files = filelist.split(',')
    result = []

    for file in files:
        root = Path("static") / projectID
        if root.is_dir():
            file_path = root / file
            print(file_path)
...
async def edit_files(projectID: str, file_edits: FileEditSchema):
    ...
    return {"message": "Files edited successfully: " + ",".join([file_edit.path for file_edit in file_edits])}


@app.get("/c/{random_string}")
async def serve_index(random_string: str):
    file_path = Path("static") / random_string / "index.html"
    print(file_path)
